...
export class Event {
    ...
    isValidIn(calendar) {
        ...
        if (calendar.events[newDate]) {
            ...
            if (event) {
                $("#errors").text(
                    `Das überschneidet sich mit dem Event '${event.title}'
        ...
        const duration =
            ...
            (1000 * 60);
        if (duration < 0) {
            $("#errors").text("Der Start kann nicht nach dem Ende sein.");
            return false;
        } else if (duration < 30) {
            $("#errors").text("Events müssen mindestens 30 min lang sein.");
            return false;
        }
        return true;
    }
